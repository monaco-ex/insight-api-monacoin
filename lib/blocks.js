...
Networks.livenet = Networks.add(require('@cryptcoin-junkey/coininfo')('MONA').toBitcore());
...
BlockController.prototype.getPoolInfo = function(block) {
  var input = block.transactions[0].inputs[0]

  if (input) {
    var coinbaseBuffer = input._scriptBuffer;
...
BlockController.prototype.getBlockReward = function(height) {
  var halvings = Math.floor(height / 1051200);
  // Force block reward to zero when right shift is undefined.
  if (halvings >= 64) {
    return 0;
  }

  // Subsidy is cut in half every 1,051,200 blocks which will occur approximately every 3 years.
